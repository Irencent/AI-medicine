...
def data_normalization_apply_cohort1_to_all(data):
    ...
    data_base_feature, data_base_ch_num = data_base_cohort.iloc[:, 3:].astype('float'), data_base_cohort.iloc[:, 1]
    data_feature, data_ch_num = data.iloc[:, 3:].astype('float'), data.iloc[:, 1]
    ...
    data = pd.concat([data.iloc[:, 0], data_ch_num, data.iloc[:, 2], data_feature], axis=1)
    return data


def lgb_evaluate_lgbm_old(max_depth, num_leaves, learning_rate, max_bin, colsample_bytree, reg_alpha, reg_lambda,
    ...
    p_lg_new = clf_lg.predict_proba(X_new) if multi_cls else clf_lg.predict_proba(X_new)[:, 1]
    ...
    lg_auc_new = roc_auc_score(y_new, p_lg_new, multi_class='ovr') if multi_cls else roc_auc_score(y_new, p_lg_new)
    ...
    print('New AUC : ', np.round(lg_auc_new, decimals=4))
...
def lgb_evaluate_rdforest(n_estimators, max_depth, min_samples_split, min_samples_leaf, min_weight_fraction_leaf, max_leaf_nodes, min_impurity_decrease, verbose, ccp_alpha, max_samples, X_train, y_train, X_test, y_test, X_val, y_val, multi_cls=False):

     # Set up the parameters for the random forest classifier. The first 10 arguments are various hyperparameters.
    params = {'max_depth': int(max_depth),
    ...
    # Initialize the RandomForestClassifier and set its parameters
    modellgb = RandomForestClassifier()
    modellgb.set_params(**params)
...
def lgb_evaluate_mlp(alpha, batch_size, learning_rate_init, power_t, max_iter, random_state, tol, momentum, validation_fraction, beta_1, beta_2, epsilon, n_iter_no_change, max_fun, X_train, y_train, X_test, y_test, X_val, y_val, multi_cls=False):
    # Set up the parameters for the random forest classifier. The first 15 arguments are various hyperparameters.
    params = {'alpha': alpha,
    ...
    # Initialize the MLPClassifier and set its parameters
    modellgb = MLPClassifier()
    modellgb.set_params(**params)
